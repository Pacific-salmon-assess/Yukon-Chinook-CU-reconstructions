...
fitRR <- function( ctlFile="R/fittedMod/estControlFile.txt",
                   arrivSD=NULL,
                   folder="R/fittedMod",
                   simData=NULL,
                   saveRun=TRUE,
                   runRW=1 )
{
  ...
  load(here("data/generated/chinookYkData.Rdata"))
  ...
  if("yukonChinookRunRecon" %in% names(getLoadedDLLs()))
    dyn.unload(dynlib(here("R/yukonChinookRunRecon")))          # unlink the C++ code if already linked
  # Compile the model
  compile(here("R/yukonChinookRunRecon.cpp"), flags = "-g")
  # Load the DLL
  dyn.load(dynlib(here("R/yukonChinookRunRecon")))          # Dynamically link the C++ code
