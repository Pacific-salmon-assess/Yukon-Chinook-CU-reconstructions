...
library(tidyverse)
library(ggsidekick)
source(here("R/initRR.R"))

lastyear <- "2025"

# generate data file if inputs updated
source(here("R/procData.R"))
processData()
...
load(here("R/fittedMod/rpt.Rdata"))

# process RR model output and save time-series of spawner abundance
cdn_harvest <- read.csv(here("data/YkCk_Harvest_CA_Data.csv")) %>%
...
mcquesten_cont <- read.csv(here("data/mcquesten.csv"))
...
year_index <- length(seq(1985,lastyear))

mcq_counts <- matrix(NA, 1000,year_index)
...
for(i in 1:1000) { #
  mcq_count <- rnorm(n=year_index,mean=mu_border_total,sd=sd_border_total)*rnorm(year_index,mu_mcq_prop,sd_mcq_prop)
...
CU_border_upr <- matrix(data=Rse$uCI, nrow=8, ncol=year_index)
CU_border_lwr <- matrix(data=Rse$lCI, nrow=8, ncol=year_index)
...
colnames(CU_spwn_upr) <- seq(1985,as.numeric(lastyear))
...
colnames(CU_spwn_lwr) <- seq(1985,as.numeric(lastyear))
...
big_salmon <- read.csv(here("data/trib-spwn.csv")) |>
  filter(system == "bigsalmon",
         type == "sonar")

prop_comp <- mean(big_salmon$estimate/(CU_spwn[7,c(21:year_index)]), na.rm=TRUE)
prop_comp_ind <- big_salmon$estimate/CU_spwn[7,c(21:year_index)]
...
bs <- matrix(NA,1000,year_index)
my <- matrix(NA,1000,year_index)
...
big_salmon_inf_2[c(21:year_index)] <- big_salmon$estimate
...
hatch_upper <- read.csv(here("data/trib-spwn.csv")) |>
...
year <- seq(1985,as.numeric(lastyear))
big.S <- cbind(year,(big_salmon_recon))
big.S <- as.data.frame(big.S)
big.S$stock <- "Big.Salmon"
big.S$cv <- bs_cv
big.S$cv[c(21:year_index)] <- 0.05
big.S$lwr <- bs_lwr
big.S$upr <- bs_upr

big.S$lwr[21:year_index] <- (big_salmon_recon+(big_salmon_recon*0.1))[21:year_index]
big.S$upr[21:year_index]  <- (big_salmon_recon-(big_salmon_recon*0.1))[c(21:year_index)]
