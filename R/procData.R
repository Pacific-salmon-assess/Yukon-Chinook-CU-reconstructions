processData <- function()
{
	...
	counts <- read.csv(here("data/borderCounts.csv"))
	names(counts)[names(counts)=='count_type'] <- 'gear'
	counts$gear <- as.character(counts$gear)

	# -- GSI sub-stock sampling by day, n_sgyd --
	gsi <- read.csv(here("data/border-gsi-table-2025.csv")) %>%
	  rename( sample_num=fish )
	stockID <- read.csv(here("data/stockIDs.csv")) %>% arrange(plotOrder)
	...
	borderPass <- read.csv(here("data/border-passage.csv"))
	...
	save(chinookYkData, file=here('data/generated/chinookYkData.Rdata'))
}
